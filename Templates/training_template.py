...
class Application(tk.Frame):
    ...
    def print_values(self):
        ...
        for x in range(name):
            print("Word: ",self.text_name[x].get(),"  File:",self.text_filename[x].get(), "  File path: ", self.fileLocations[x])
    ...
    def autoDoThis(self):
        ...
        for i in range(len(mgaWords)):
            wordName = mgaWords[i]
            fileLocation =  mgaLocation[i]
            openPose = getKeyPoints()
            openPose.learn(fileLocation,showDisplay=False)
            self.saveToDataset(openPose.getKeyPoints(), wordName)
